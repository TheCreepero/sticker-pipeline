...
def generate_html_dashboard(items, output_html="listings.html"):
    """
    Generates a local HTML dashboard with one-click copy buttons.
    """
    html_cards = ""
    for item in items:
        html_cards += f"""
...
def run_full_pipeline(
    ...
    for idx, img_path in enumerate(png_files):
        ...
        # Strip digits, trailing underscores/hyphens, and normalize spacing for metadata
        clean_name = re.sub(r"\d+", "", base_name)
        display_name = clean_name.replace("_", " ").replace("-", " ").strip()
        display_name = re.sub(r"\s+", " ", display_name).title()
        
        # Fallback to base name if the filename was purely numeric
        if not display_name:
            display_name = base_name.replace("_", " ").replace("-", " ").strip().title()
        
        # 1. Generate 3x3 repeating sticker sheet
        sticker = Image.open(img_path).convert("RGBA")
        
        # Optional: Normalize transparent margins automatically
    ...
    # Save Interactive Browser Dashboard
    generate_html_dashboard(items_data, output_html)

    print(f"Pipeline complete! Cleaned metadata generated for {len(png_files)} files.")
